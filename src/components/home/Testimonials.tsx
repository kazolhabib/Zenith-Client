      ...
      <div className="relative w-full flex overflow-x-hidden z-10">
        ...
        >
          {duplicatedTestimonials.map((testimonial, index) => (
            <div 
              key={index}
              className="w-[280px] md:w-[360px] min-h-[360px] md:min-h-[440px] shrink-0 bg-[#121217]/50 backdrop-blur-xl p-6 md:p-8 rounded-[2rem] md:rounded-[2.5rem] shadow-[0_8px_40px_rgba(0,0,0,0.6)] border border-white/5 hover:border-brand/30 hover:-translate-y-2 transition-all duration-500 relative overflow-hidden group flex flex-col justify-between"
            >
              ...
              <div>
                ...
                <div className="flex justify-between items-center mb-4 md:mb-6">
                  ...
                  <Quote className="w-6 h-6 md:w-8 md:h-8 text-white/5 group-hover:text-brand/20 transition-colors duration-500" />
                </div>

                {/* Review Content */}
                <p className="text-slate-300 whitespace-normal leading-relaxed text-sm md:text-base font-medium group-hover:text-white transition-colors duration-300 mb-4 md:mb-6">
                  "{testimonial.content}"
                </p>

                {/* Stay Info */}
                <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/5 border border-white/5 text-[10px] md:text-[11px] font-bold text-slate-400 group-hover:text-brand-light group-hover:border-brand/20 transition-all">
              ...
              {/* User Bio */}
              <div className="flex items-center gap-3 md:gap-4 mt-6 md:mt-8 pt-4 md:pt-6 border-t border-white/5">
                <div className={`relative rounded-full p-[2px] bg-gradient-to-br ${testimonial.color} shadow-lg`}>
                  <img 
                    src={testimonial.image} 
                    alt={testimonial.name} 
                    className="w-10 h-10 md:w-12 md:h-12 rounded-full object-cover ring-2 ring-[#121217]"
                  />
                </div>
                <div>
                  <h4 className="font-bold text-white text-xs md:text-sm flex items-center gap-1 md:gap-1.5">
                    {testimonial.name}
                    {testimonial.verified && (
                      <span className="flex items-center gap-0.5 px-1.5 py-0.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-[8px] md:text-[9px] font-bold text-emerald-400">
                        <Check className="w-2.5 h-2.5" /> Verified
                      </span>
                    )}
                  </h4>
                  <p className="text-[10px] md:text-xs text-slate-500 font-bold tracking-wider uppercase mt-0.5">{testimonial.role}</p>
                </div>
              </div>
